#include "BombHeli.h"
#include "..\include\BombHeli.h"


BombHeli::BombHeli() : m_MyWorld(m_MyWorld)
{
}
...
BombHeli::BombHeli(b2World& MyWorld, b2Vec2 location, AssetManager*& manage) : m_MyWorld(MyWorld)
{

	m_health.setTexture(manage->GetTexture(O_HELI_FULL_HEALTH));
	m_health.setPosition(800, 10);
	m_emptyHealth.setTexture(manage->GetTexture(O_HELI_EMPTY_HEALTH));
	m_emptyHealth.setPosition(800, 10);
	...
	tempBody.setTexture(manage->GetTexture(O_BOMB_HELI_BODY_R));
	tempBody.setPosition(200, 200);
	tempBody.setOrigin(manage->GetTexture(O_BOMB_HELI_BODY_R).getSize().x / 2, manage->GetTexture(O_BOMB_HELI_BODY_R).getSize().y / 2);
	...
	tempb2Body->SetUserData((void*)17);
...
void BombHeli::CreateGun()
{
	sf::Sprite tempGun;
	b2Body* tempb2Gun;

	tempGun.setTexture(manage->GetTexture(O_BOMB_HELI_GUN));
	tempGun.setPosition(GetHeliBody().getPosition().x - 50, GetHeliBody().getPosition().y);
	tempGun.setOrigin(manage->GetTexture(O_BOMB_HELI_GUN).getSize().x / 2, manage->GetTexture(O_BOMB_HELI_GUN).getSize().y / 2);
...
void BombHeli::CreatePropeller()
{
	...
	tempPropeller.scale(1.1, 1.1);
	SetHeliPropeller(tempPropeller);
}

bool BombHeli::getStatus()
{
	return m_Killed;
}

void BombHeli::SpawnHeli(sf::RenderWindow& window)
{


	for (b2Body* BodyIterator = m_MyWorld.GetBodyList(); BodyIterator != 0; BodyIterator = BodyIterator->GetNext())
	{
		if ((int)BodyIterator->GetUserData() == 17) //Heli Body
		{
			SetPositionBodyHelper(SCALE * BodyIterator->GetPosition().x, SCALE * BodyIterator->GetPosition().y-70);
...
void BombHeli::movingHeli(const int time)
{
	...
	if (temp.getPosition().x >= R_SCREEN)
	{
		...
		temp.setTexture(manage->GetTexture(O_BOMB_HELI_BODY_L));
		temp.setRotation(-20);
		tempPropeller.setRotation(-20);
		PosGun_X = -20;
	...
	{
		speed = SPEED_BOMB_HELI;
		temp.setTexture(manage->GetTexture(O_BOMB_HELI_BODY_R));
		temp.setRotation(20);
		tempPropeller.setRotation(20);
		PosGun_X = 30;
...
void BombHeli::Shooter(sf::RenderWindow& window, sf::Clock& time, sf::Vector2f& pos)
{
	b2Vec2 PlayerPos = { pos.x / SCALE, pos.y / SCALE };
	if (time.getElapsedTime().asSeconds() >= 0.4)
	{
		time.restart().asSeconds();
		Bullet NewBullet;
		NewBullet.CreateBullets(m_MyWorld, this->getb2Body()->GetPosition(), PlayerPos, false, 0.2);
	}
